  ...
  function(recordModulo, runtime, file, encode, search, format, log, config, libreriaGeneral, render, xml) {
    ...
    function execute(scriptContext) {
      try {
        ...
        */
        ...
        var date = parseDateTo(new Date(), 'DATE');
        var currentTime = parseDateTo(new Date(), 'DATETIME');

        log.debug( "date - currentTiem", { date:date, currentTime:currentTime } )
    ...
    function obtenerPdf() {
      ...
      if (ArrLibroDiario.length != null && ArrLibroDiario.length != 0) {
        ...
        pdfString += "</head>";
        pdfString += "<body font-size=\"8\" size=\"A4-LANDSCAPE\">";
        ...
        pdfString += "<table width=\"100%\"><tr><td align =\"center\">" + /*GLOBAL_LABELS["Alert1"][language]+*/ "</td></tr>";
    ...
    function parseDateTo(trandate, type) {
      var $date = '';
      if (!trandate) return;
      $date = format.parse({ value: trandate, type: format.Type.DATE });
      $date = format.format({ value: $date, type: format.Type[type] });
      return $date;
    }
