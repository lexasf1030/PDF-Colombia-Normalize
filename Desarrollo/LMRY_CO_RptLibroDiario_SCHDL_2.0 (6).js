 ...
 * 2.00       6 dic 2021     LatamReady Consultor
 * File: LMRY_CO_RptLibroDiario_SCHDL_2.0.js
 */
/**
 * @NApiVersion 2.0
  ...
  function(recordModulo, runtime, file, encode, search, format, log, config, libreriaGeneral, render, xml) {
    ...
    function execute(scriptContext) {
      try {
        ...
        */
        ...
        log.debug('[execute] ArrLibroDiario', ArrLibroDiario.length);
        ObtieneAccountingContext(); //Obtengo el numero de cuenta
        log.debug("[execute] arrAccountingContext", arrAccountingContext);
        ObtieneAccountingContextNumeroFijo(); //Obtengo los valores del puc
        log.debug("[execute] arrAccountingContextCuentaEnlazada", arrAccountingContextCuentaEnlazada);
        ...
        log.error('Error', err);
        log.error('Line Error', err.lineNumber);
        log.error('Stack Error', err.stack);
        return true;
      }
    }
    ...
    function obtenerParametrosYFeatures() {
      ...
      try {
        paramsubsidi = objContext.getParameter({
          name: 'custscript_lmry_co_librodm_subsi'
        });
        paramperiodo = objContext.getParameter({
          name: 'custscript_lmry_co_librodm_period'
        });
        paramidrpt = objContext.getParameter({
          name: 'custscript_lmry_co_librodm_idreport'
        });
        paramMultibook = objContext.getParameter({
          name: 'custscript_lmry_co_librodm_multi'
        });
        paramEndPeriod = objContext.getParameter({
          name: 'custscript_lmry_co_adjust_diario'
        });
        paramFormatoReporte = objContext.getParameter({
          name: 'custscript_lmry_co_librodm_formatoreport'
        });

        log.debug('[obtenerParametrosYFeatures] Parametros', paramsubsidi + ', ' + paramperiodo + ', ' + paramidrpt + ', ' + paramMultibook + ', ' + paramEndPeriod + ', ' + paramFormatoReporte);
        ...
        log.error('[obtenerParametrosYFeatures] Error', err)
      }
    }
    ...
    function obtenerPdf() {
      ...
      if (ArrLibroDiario.length != null && ArrLibroDiario.length != 0) {
        ...
        pdfString += "<table width=\"100%\"><tr><td colspan=\"5\" align =\"center\">" + GLOBAL_LABELS["Alert1"][language] + "</td></tr>";
        pdfString += "<tr>";
        pdfString += "<td colspan=\"5\" align =\"center\" width=\"40%\"><b> " + GLOBAL_LABELS["Alert2"][language] + " " + xml.escape(companyname) + "</b></td>";
        pdfString += "</tr>";
        pdfString += "<tr>";
        pdfString += "<td colspan=\"5\" align =\"center\" width=\"60%\"><b>NIT :" + companyruc + "</b></td>";
        pdfString += "</tr>";
        pdfString += "<tr>";
        pdfString += "<td colspan=\"5\" align =\"center\" width=\"40%\"><b>" + GLOBAL_LABELS["Alert3"][language] + periodstartdate + GLOBAL_LABELS["alConnector"][language] + periodenddate + "</b></td>";
        pdfString += "</tr>";
        if ((feamultibook || feamultibook == 'T') && (paramMultibook != '' && paramMultibook != null)) {
          pdfString += "<tr>";
          pdfString += "<td colspan=\"5\" align =\"center\" width=\"60%\"><b>" + GLOBAL_LABELS["Alert4"][language] + multibook_name + "</b></td>";
          pdfString += "</tr>";
        }

        pdfString += "<tr>";
        pdfString += "<td colspan=\"5\" align =\"center\" width=\"60%\"><b>" + GLOBAL_LABELS["origin"][language] + "Netsuite" + "</b></td>";
        pdfString += "</tr>";
        pdfString += "<tr>";
        pdfString += "<td colspan=\"5\" align =\"center\" width=\"60%\"><b>" + GLOBAL_LABELS["date"][language] + todays + "</b></td>";
        pdfString += "</tr>";
        pdfString += "<tr>";
        pdfString += "<td colspan=\"5\" align =\"center\" width=\"60%\"><b>" + GLOBAL_LABELS["time"][language] + currentTime + "</b></td>";
        pdfString += "</tr>";
        ...
        pdfString += "<td colspan=\"1\" width=\"10%\" style=\"font-size: 1em\" ><b>" + GLOBAL_LABELS["Alert7"][language] + "</b></td>";
        pdfString += "<td colspan=\"1\" width=\"30%\" style=\"font-size: 1em\" align =\"right\"><b>" + GLOBAL_LABELS["Alert8"][language] + "</b></td>";
        ...
        for (var i = 0; i <= ArrLibroDiario.length - 1; i++) {
          ...
          var primeraCabecera = ArrLibroDiario[i][0] === primerDia && flag === 0;
          ...
          if (fechaActual != ArrLibroDiario[i][0]) {
    ...
    function ObtieneLibroDiario() {
      ...
      if ((feamultibook || feamultibook == 'T')) {
        ...
        var savedsearch2 = search.create({
          ...
          filters: [
            ["posting", "is", "T"],
            "AND", ["formulatext: {account.custrecord_lmry_co_puc_d6_description}", "isnotempty", ""],
            "AND", ["formulanumeric: CASE {account.custrecord_lmry_localbook} WHEN 'T' THEN 1 ELSE 0 END", "equalto", "1"],
      ...
      if (featAccountingSpecial || featAccountingSpecial == 'T') {
        ...
        pagedData.pageRanges.forEach(function(pageRange) {
          ...
          page.data.forEach(function(result) {
            ...
            log.debug('[obtenerPeriodosEspeciales-1] periodname', periodname);
          })
        });
      } else {
        if (paramperiodo != null && paramperiodo != '') {
          ...
          log.debug('[obtenerPeriodosEspeciales-2] periodname', periodname);
        }
      }
            ...
            for (var i = 0; i < intLength; i++) {
              ...
              for (var col = 0; col < columns.length; col++) {
                if (col == 3) {
                  ...
                  if (objResult[i].getValue(columns[col]) != null && objResult[i].getValue(columns[col]) != '- None -' && objResult[i].getValue(columns[col]) != 'NaN' && objResult[i].getValue(columns[col]) != 'undefined')
