...
define(["N/record", "N/runtime", "N/file", "N/encode", "N/search",
    ...
    function (recordModulo, runtime, fileModulo, encode, search, format, log,
        ...
        function execute(context) {
            try {
                ...
                if (PeriodosRestantes.length != 0) {
                    ...
                    log.debug('ArrMovimientos tam', ArrMovimientos.length);
                ...
                if (paramPUC == '9') {
                    ...
                    log.debug('dataTotal sin ceros', dataTotal);
                    if (paramDigits == 2) {
                        var arr6digits = [];
        ...
        function juntarSaldoYMovimiento(arrDataSaldo, arrDataMovimiento) {
            log.debug('inicio union de data', 'inicia union de data')
            var arrTotal = arrDataSaldo;
            log.debug('arrTotal temporal', arrTotal);
            log.debug('arrDataMovimiento temporal', arrDataMovimiento)
            ...
            //log.debug('arrTotal', arrTotal);
            log.debug('arrDataMovimiento 2', arrDataMovimiento);
            ...
            log.debug('arrTotal fin temp', arrTotal);
                        ...
                        if (paramDigits == 2) {
                            for (var col = 0; col < columns.length - 2; col++) {
                                if (featMulti) {
                                    arrQuiebre[0] = objResult[i].getValue(columns[14]);
                                    arrQuiebre[1] = objResult[i].getValue(columns[15]);
                                    // test = regex.test(objResult[i].getValue(columns[14]));
                                } else {
                                    arrQuiebre[0] = objResult[i].getValue(columns[13]);
                                    arrQuiebre[1] = objResult[i].getValue(columns[14]);
                                    // test = regex.test(objResult[i].getValue(columns[13]));
                                }
                                if (col == 7) {
                                    if (featMulti) {
                                        arrQuiebre[9] = redondear(objResult[i].getValue(columns[12]));
                            ...
                            if (arrQuiebre[9] && arrQuiebre[9] != 0) {
                            ...
                            if (featMulti) {
                                ...
                                arrQuiebre[2] = objResult[i].getValue(columns[14]);
                                arrQuiebre[3] = objResult[i].getValue(columns[15]);
                                // test = regex.test(objResult[i].getValue(columns[16]));
                            } else {
                                ...
                                // test = regex.test(objResult[i].getValue(columns[15]));
                            }
                            ...
                            log.debug('arrQuiebre[11]', arrQuiebre[11]);
                            if (arrQuiebre[11] != 0) {
